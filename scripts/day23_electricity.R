...
data_c <- data  %>% rename(access = Access.to.electricity....of.population., country = Entity)  %>% 
  ...
  group_by(Code) %>% mutate(n_data = n()) %>% filter(n_data == 27)
...
ggsave(p1, file="chartchall_d23.png", device="png",dpi = 250, width = 70, height = 85, units = "cm") # can adjust resolution
...
ggsave(p2, file="chartchall_d23_2.png", device="png",dpi = 400, width = 25, height = 15, units = "cm") # can adjust resolution
